...
public class SiirtotiedostoPalvelu extends Dokumenttipalvelu {
   ...
   *     allowed formats: '2024-01-24T10:15:30', '24.01.2024 10.15.30'. Value is optional, can be
   *     null or empty.
   * @param timeRangeEnd Endpoint of the data timerange contained by this siirtotiedosto allowed
   *     formats: '2024-01-24T10:15:30', '24.01.2024 10.15.30'. Value is optional, can be null or
   *     empty.
  ...
  public ObjectMetadata saveSiirtotiedosto(
    ...
    Optional<LocalDateTime> timeRangeStartVal =
        StringUtils.isEmpty(timeRangeStart)
            ? Optional.empty()
            : Optional.of(LocalDateTime.parse(timeRangeStart, dateTimeFormatter));
    Optional<LocalDateTime> timeRangeEndVal =
        StringUtils.isEmpty(timeRangeEnd)
            ? Optional.empty()
            : Optional.of(LocalDateTime.parse(timeRangeEnd, dateTimeFormatter));
